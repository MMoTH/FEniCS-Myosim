...
n_array_length = 23 # for 2-state model
...
y_vec_new = Myosim.implement_time_step(y_vec_old, delta_hsl, hsl, p_f, cb_f, t_a.t_a)

#y_vec_old = y_vec_new

#stress.vector()[:] = np.array(Myosim.Get_cb_force_vec())
...
#hsl_new = project(0.5*sqrt(dot(f0, Cmat*f0))*ls0, Quad).vector().get_local()[:]

#delta_hsl = hsl_new - hsl_old
