...
for l in range(10):
    ...
    if l >= 0 and l < 5:
        ...
        u_D.u_D += 0.001
        #stress.vector()[:] = np.ones(24) * 1
        stress.vector()[:] = np.arange(24)
        
    
        
    #displacements.append(u_D.u_D)
    
    solve(Ftotal == 0, w, bcs, J = Jac, form_compiler_parameters={"representation":"uflacs"})
    
    hsl = project(sqrt(dot(f0, Cmat*f0)), Quad).vector().get_local()[:] 
    
    hsls[l,:] = hsl
      
    #hsl = project(sqrt(dot(f0, Cmat*f0))*1000, Quad).vector().get_local()[:]
...
np.save("/home/fenics/shared/hsls",hsls)
np.save("/home/fenics/shared/p_fs_24",stresses)

#plt.plot(range(400), displacements, 'x')
#plt.plot(range(400), displacements, 'x')
