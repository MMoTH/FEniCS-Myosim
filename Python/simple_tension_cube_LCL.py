...
for nstep in range(0, tstep):
    ...
    T = uflforms.Cauchy()
    # By right Cauchy stress should be defined based on the current configuration basis right ? (LCL) #
    F = uflforms.Fmat()
    f = F*f0/sqrt(inner(F*f0, F*f0))
    Tff =  inner(f,T*f)
    #Tff =  inner(f0,T*f0)
    # LCL workaround ################################
    temp_DG = project(Tff, FunctionSpace(mesh, "DG", 1), form_compiler_parameters={"representation":"uflacs"}) 
    temp = interpolate(temp_DG, Quad).vector().get_local()[0]
    print len(interpolate(temp_DG, Quad).vector().get_local()), len(ls)
    #temp = project(Tff, Quad).vector().get_local()[0]
    print'Cauchy fiber stress interpolate = ', temp #, "projection = ", project(Tff, Quad).vector().get_local()[0]
