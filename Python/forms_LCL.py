...
class Forms(object):
    ...
    def Cauchy(self):
        ...
        Stress_local = as_tensor([[Sff, Sfs, Sfn], [Sfs, Sss, Sns], [Sfn, Sns, Snn]]) 
        
        TransMatrix = as_tensor(f0[i]*e1[j], (i,j)) + as_tensor(s0[i]*e2[j], (i,j)) + as_tensor(n0[i]*e3[j], (i,j))
        
        Stress_global = as_tensor(TransMatrix[i,k]*TransMatrix[j,l]*Stress_local[k,l],(i,j))
        
        Stress_global = Stress_local
        
        T = (1.0/J)*F*Stress_global*F.T - p*I
        #T = F*Stress_global - p*inv(F.T)
        
        # alternatively use C++ to calculate stress
        '''Elocal = as_tensor([[Eff, Efs, Efn], [Esf, Ess, Esn], [Enf, Ens, Enn]])
        TF = TensorFunctionSpace(mesh, 'DG', 1)
        Etrans = project(Elocal,TF)
        header_file5 = open("Exp_CPP/stresscode_exp.cpp","r")
        stresscode_exp = header_file5.read()
        ...
        for e in Telem2.sub_elements():
            quad_scheme = 'default'
        StressTensor_Exp = dolfin.Expression(stresscode_exp, element=Telem2)
        StressTensor_Exp.U = Etrans
        TransMatrix = as_tensor(f0[i]*e1[j], (i,j)) + as_tensor(s0[i]*e2[j], (i,j)) + as_tensor(n0[i]*e3[j], (i,j))
        StressGlobal_Exp = as_tensor(TransMatrix[i,k]*TransMatrix[j,l]*StressTensor_Exp[k,l],(i,j))
        Second_PK = StressGlobal_Exp
        T = 1.0/J*F*Second_PK*F.T - p*I'''
