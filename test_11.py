...
os.system("rm ./test_11/*.pvd")
os.system("rm ./test_11/*.vtu")
...
displacementfile = File("./test_11/u_disp.pvd")

if(MPI.rank(comm) == 0):
    fdataPV = open("./test_11/PV_.txt", "w", 0)
...
tarray = []
hslarray = np.zeros((no_of_time_steps,no_of_int_points))
calarray = np.zeros((no_of_time_steps,no_of_int_points))
strarray = np.zeros((no_of_time_steps,no_of_int_points))
pstrarray = np.zeros((no_of_time_steps,no_of_int_points))
...
aorta_resistance = 10e3
aorta_compliance = 1e-6 / 1000

arteries_resistance = 30e3
arteries_compliance = 2.5e-7 / 1000

arterioles_resistance = 30e3
arterioles_compliance = 1.5e-7 / 1000

capillaries_resistance =30e3
capillaries_compliance = 0.8e-6 / 1000

veins_resistance = 10e3
veins_compliance = 1.2e-4 / 1000

ventricle_resistance = 1e3
...
#initial_ventricular_volume = 1.0 * ventricle_slack_volume
initial_ventricular_volume = LVCavityvol.vol
volume = np.zeros(no_of_compartments)
volume[-2] = blood_volume - initial_ventricular_volume
volume[-1] = initial_ventricular_volume

dumped_populations = np.zeros((no_of_time_steps, no_of_int_points, n_array_length))
...
while(cycle < cycles):
    ...
    if (pressure[-1] > pressure[0]):
        flows[0] = \
            (pressure[-1] - pressure[0]) / resistance[0]
    
    flows[1] = \
        (pressure[0] - pressure[1]) / resistance[1]
    
    flows[2] = \
        (pressure[1] - pressure[2]) / resistance[2]
    
    flows[3] = \
        (pressure[2] - pressure[3]) / resistance[3]
    
    flows[4] = \
        (pressure[3] - pressure[4]) / resistance[4]
    
    flows[5] = 0.0
    if (pressure[4] > pressure[-1]):
        flows[5] = \
            (pressure[4] - pressure[-1]) / resistance[-1]
            

    if(MPI.rank(comm) == 0):
        
            print "ventricle_to_aorta = ", flows[0] ;
            print "aorta_to_arteries = ", flows[1] ;
            print "arteries_to_arterioles = ", flows[2] ;
            print "arterioles_to_capillaries = ", flows[3] ;
            print "capillaries_to_veins = ", flows[4] ;
            print "veins_to_ventricle = ", flows[-1] ;

    volume[0] = volume[0] + step_size*(flows[0] - flows[1])
    volume[1] = volume[1] + step_size*(flows[1] - flows[2])
    volume[2] = volume[2] + step_size*(flows[2] - flows[3])
    volume[3] = volume[3] + step_size*(flows[3] - flows[4])
    volume[4] = volume[4] + step_size*(flows[4] - flows[5])
    volume[5] = volume[5] + step_size*(flows[5] - flows[0])
...
np.save("/home/fenics/shared/test_11/rates",rate_constants)

np.save("/home/fenics/shared/test_11/dumped_populations",dumped_populations)

np.save("/home/fenics/shared/test_11/tarray",tarray)

np.save("/home/fenics/shared/test_11/stress_array",strarray)

np.save("/home/fenics/shared/test_11/pstress_array",pstrarray)

np.save("/home/fenics/shared/test_11/calcium",calarray)

np.save("/home/fenics/shared/test_11/HSL",hslarray)
