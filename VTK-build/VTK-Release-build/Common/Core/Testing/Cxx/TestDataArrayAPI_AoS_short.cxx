...
int Test_ComponentNames()
{
  ...
  for (vtkIdType c = 0; c < static_cast<vtkIdType>(comps); ++c)
  {
    ...
    DataArrayAPIAssert(array->GetComponentName(c) == nullptr,
...
int Test_voidPtr_GetVoidPointer()
{
  ...
  for (vtkIdType t = 0; t < tuples; ++t)
  {
    ...
    DataArrayAPIAssert(ptr != nullptr, "GetVoidPointer returned nullptr!");
    for (int c = 0; c < comps; ++c)
    {
      ScalarT test = *ptr++;
...
struct CheckShallowCopy
{
  ...
  bool operator()(vtkGenericDataArray<Array1DerivedT, Array1ValueT> *,
                  vtkGenericDataArray<Array2DerivedT, Array2ValueT> *)
  {
    return true;
  }
...
// Just test that the returned iterator is non-nullptr and the data types match.
template <typename ScalarT, typename ArrayT>
int Test_vtkArrayIteratorPtr_NewIterator()
{
  ...
  DataArrayAPIAssert(iter != nullptr,
                     "NewIterator() returns nullptr.");
...
// GetLookupTable should be non-nullptr after calling.
template <typename ScalarT, typename ArrayT>
int Test_void_CreateDefaultLookupTable()
{
  ...
  if (source->GetLookupTable() == nullptr)
  {
...
int Test_void_InsertNextTuple_tuple()
{
  ...
  for (vtkIdType t = 0; t < tuples; ++t)
  {
    ...
    vtkIdType result = source->InsertNextTuple(&tuple[0]);
    if (result != t)
    {
      DataArrayAPIError("Return value should be " << t << ", but got " << result
                        << ".");
    }
    if (source->GetSize() < ((t + 1) * comps))
    {
...
int ExerciseDataArray()
{
  ...
  errors += Test_void_FillComponent_j_c<ScalarT, ArrayT>();
